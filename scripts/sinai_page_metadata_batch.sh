...
while read MS; do
    ...
    awk -F, -v OFS=, '{$4=NR}1' /$targetDIR/${PWD##*/}"_temp3.csv" > /$targetDIR/${PWD##*/}"_temp4.csv" && awk -F"," 'BEGIN { OFS = "," } {$5=""; $6=""; $7=""; $8=""; $9="ChildWork"; $10="Images: Contact the Monastery of St. Catherine’s of the Sinai. Metadata: Unless otherwise indicated all metadata associated with this manuscript is copyright the authors and released under Creative Commons Attribution 4.0 International License."; $11="discovery"; print}' /$targetDIR/${PWD##*/}"_temp4.csv" > /$targetDIR/${PWD##*/}"_temp5.csv" && awk -F, '{print $1,$4,$12,$2,$5,$6,$7,$8,$9,$10,$3}' OFS=, /$targetDIR/${PWD##*/}"_temp5.csv" > /$targetDIR/${PWD##*/}"_temp6.csv" && { echo 'File Name,Item Sequence,Visibility,Title,IIIF Range,viewingHint,Parent ARK,Item ARK,Object Type,Rights.statementLocal,Source'; cat /$targetDIR/${PWD##*/}"_temp6.csv"; } > /$targetDIR/${PWD##*/}"_temp7.csv"
